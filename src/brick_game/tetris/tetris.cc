...
void check_level(){
    if (game_.score % TO_NEXT_LEVEL == 0){
        game_.level++;
        if (game_.speed > 100) game_.speed -= 100;
    }
}
...
char is_in_the_area(unsigned i, unsigned j){
    ...
    char a = 0;
    char b = 0;
    if (i >= tetris_.y_current_figure && i < tetris_.y_current_figure + tetris_.current_figure->row) a = 1;
    if (j >= tetris_.x_current_figure && j < tetris_.x_current_figure + tetris_.current_figure->col) b = 1;
    if (a == 1 && b == 1) {
        size_t shift_x = tetris_.x_current_figure;
        size_t shift_y = tetris_.y_current_figure;
        int x = j - shift_x;
        int y = i - shift_y; 
        if (tetris_.current_figure->body[y][x] == 1) result = 1;
    }
    return result;
}
